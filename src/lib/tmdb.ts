 ...
 *
 * Server-only: import image helpers from `@/lib/tmdb-images` in client code.
 */
import "server-only";
...
export { getBackdropUrl, getImageUrl, getPosterUrl, getProfileUrl, IMAGE_BASE };
...
/** Single-request movie payload (append_to_response). */
export type TmdbMovieDetails = TmdbMovie & {
  credits?: TmdbCredits;
  videos?: { results: TmdbVideo[] };
  similar?: TmdbPaged<TmdbMovie>;
  reviews?: TmdbReviewPage;
...
async function tmdbFetch<T>(path: string, params: Record<string, string | number> = {}): Promise<T> {
  ...
  // Prefer v4 Bearer token if available (more secure, recommended by TMDB).
  // Otherwise fall back to v3 api_key query param.
  ...
  try {
    ...
    if (res.status === 429) {
      // Rate limited by TMDB — retry after a short delay
...
/** One TMDB round-trip for the detail page (deduped per request via React cache). */
...
export const discoverMovies = (
  ...
  const releaseYear = params.primaryReleaseYear ?? params.year;
  if (releaseYear) query.primary_release_year = releaseYear;
...
function buildMovieDetails(id: number): TmdbMovieDetails {
  const movie = buildMovie(id);
  ...
  return {
    ...movie,
    credits: { cast: buildCast(id), crew: buildCrew(id) },
    videos: {
      results: [
        {
          id: `v${id}-1`,
    ...
    similar: buildPaged(
      Array.from({ length: 12 }, (_, i) => buildMovie(id * 100 + i + 1)),
      1,
      5,
    ),
    reviews: {
          ...
          flatrate: [
            {
              provider_id: 335,
              provider_name: "BluTV",
              logo_path: "/seGSXajazLMCKOS0p8Yv4R1eaJ9.jpg",
              display_priority: 1,
            },
          ],
        },
...
function getMockResponse<T>(path: string, params: Record<string, string | number>): T {
  ...
  if (path === "/discover/movie") {
    ...
    const releaseYear = Number(params.primary_release_year ?? 0);
    ...
    if (releaseYear) {
      movies = movies.filter((m) => m.release_date.startsWith(String(releaseYear)));
      if (movies.length === 0) movies = listOf(20);
    }
  ...
  if (path.startsWith("/movie/")) {
    const id = Number(path.split("/")[2]);
    return buildMovieDetails(id) as unknown as T;
  }
